...
protected:
	...
	void select_population_SO(const thisGenerationType &g,thisGenerationType &g2)
	{
		...
		vector<int> blocked;
		// TODO
		// causes segf if elite count is greater than n generation (chromosomes count)
		// ok so this is a mod of the lib, i will also modifie my code so this prbly wont happen anymore if one updates this or so
		auto eliteUpperLim = std::min(static_cast<long>(g.chromosomes.size()), static_cast<long>(elite_count));
		//for(int i=0;i<elite_count;i++)
		for(int i=0;i<eliteUpperLim;i++)
		{
